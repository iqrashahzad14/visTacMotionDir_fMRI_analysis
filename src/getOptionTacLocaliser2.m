...
function opt = getOptionTacLocaliser2()
 ...
 if nargin < 1
  ...
  % task to analyze
  opt.taskName = 'tactileLocalizer2'; % auditoryLocalizer
  ...
  opt.result.Steps(1).Contrasts(5).p = 0.005;
  ...
  opt.result.Steps(1).Contrasts(6).p = 0.01;
  opt.result.Steps(1).Contrasts(6).k = 0;
  
  opt.result.Steps(1).Contrasts(7).Name = 'motion_gt_static';
  opt.result.Steps(1).Contrasts(7).MC =  'none';
  opt.result.Steps(1).Contrasts(7).p = 0.05;
